/**
 * Side view renderers - renders the character from the left or right side
 */
 ...
 *   RightLeg: (2, 20)  4x12
 */
export async function renderSkinRightSide(
  canvas: ICanvas,
...
/**
 * Render the player's left side view onto the given canvas.
 *
 * Layout (8x32 MC pixels):
 *   Head:     (0, 0)   8x8  (head depth = 8)
 *   Body:     (2, 8)   4x12 (body depth = 4, centered in head width)
...
export async function renderSkinLeftSide(
  ...
  drawFlatBodyParts(
    ...
    skin,
    [
      { partName: "leftLeg", face: "left", x: bodyOffsetX, y: 20 },
      { partName: "body", face: "left", x: bodyOffsetX, y: 8 },
      { partName: "leftArm", face: "left", x: bodyOffsetX, y: 8 },
      { partName: "head", face: "left", x: 0, y: 0 },
    ],
    showOverlay,
    scale,
    inflated,
